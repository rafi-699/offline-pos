...
public class RNDeviceModule extends ReactContextBaseJavaModule {
    ...
    public boolean isEmulatorSync() {
        if (Build.FINGERPRINT.startsWith("generic") || Build.FINGERPRINT.startsWith("unknown") || Build.MODEL.contains("google_sdk") || Build.MODEL.toLowerCase(Locale.ROOT).contains("droid4x") || Build.MODEL.contains("Emulator") || Build.MODEL.contains("Android SDK built for x86") || Build.MANUFACTURER.contains("Genymotion") || Build.HARDWARE.contains("goldfish") || Build.HARDWARE.contains("ranchu") || Build.HARDWARE.contains("vbox86") || Build.PRODUCT.contains(ServerProtocol.DIALOG_PARAM_SDK_VERSION) || Build.PRODUCT.contains("google_sdk") || Build.PRODUCT.contains("sdk_google") || Build.PRODUCT.contains("sdk_x86") || Build.PRODUCT.contains("vbox86p") || Build.PRODUCT.contains("emulator") || Build.PRODUCT.contains("simulator") || Build.BOARD.toLowerCase(Locale.ROOT).contains("nox") || Build.BOOTLOADER.toLowerCase(Locale.ROOT).contains("nox") || Build.HARDWARE.toLowerCase(Locale.ROOT).contains("nox") || Build.PRODUCT.toLowerCase(Locale.ROOT).contains("nox") || Build.SERIAL.toLowerCase(Locale.ROOT).contains("nox")) {
            return true;
        }
    ...
    public double getBatteryLevelSync() {
        ...
        return powerStateFromIntent == null ? ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE : powerStateFromIntent.getDouble(BATTERY_LEVEL);
    }
    ...
    public boolean isWiredHeadphonesConnectedSync() {
        return ((AudioManager) getReactApplicationContext().getSystemService(MimeTypes.BASE_TYPE_AUDIO)).isWiredHeadsetOn();
    }
    ...
    public boolean isBluetoothHeadphonesConnectedSync() {
        return ((AudioManager) getReactApplicationContext().getSystemService(MimeTypes.BASE_TYPE_AUDIO)).isBluetoothA2dpOn();
    }
    ...
    public String getDeviceNameSync() {
        ...
        String string2;
        try {
            if (Build.VERSION.SDK_INT > 31 || (string2 = Settings.Secure.getString(getReactApplicationContext().getContentResolver(), "bluetooth_name")) == null) {
                return (Build.VERSION.SDK_INT < 25 || (string = Settings.Global.getString(getReactApplicationContext().getContentResolver(), "device_name")) == null) ? "unknown" : string;
            }
            return string2;
        } catch (Exception unused) {
            return "unknown";
    ...
    public String getUserAgentSync() {
        try {
            ...
            return System.getProperty(SystemProperties.HTTP_AGENT);
        }
    }
    ...
    public void getAppSetId(final Promise promise) {
        ...
        if (Build.VERSION.SDK_INT >= 34) {
            try {
                AppSetIdManager.get(getReactApplicationContext()).getAppSetId(getReactApplicationContext().getMainExecutor(), new OutcomeReceiver<AppSetId, Exception>() { // from class: com.learnium.RNDeviceInfo.RNDeviceModule.5
                    @Override // android.os.OutcomeReceiver
                    public void onResult(AppSetId appSetId) {
                        System.err.println("RNDI: AppSetId success.");
                        WritableMap writableMapCreateMap = Arguments.createMap();
                        writableMapCreateMap.putString("id", appSetId.getId());
                        writableMapCreateMap.putInt("scope", appSetId.getScope());
                    ...
                    public void onError(Exception exc) {
                        ...
                        WritableMap writableMapCreateMap = Arguments.createMap();
                        writableMapCreateMap.putString("id", "unknown");
                        writableMapCreateMap.putInt("scope", -1);
                        promise.resolve(writableMapCreateMap);
                    }
                });
                return;
        ...
        writableMapCreateMap2.putString("id", "unknown");
        writableMapCreateMap2.putInt("scope", -1);
