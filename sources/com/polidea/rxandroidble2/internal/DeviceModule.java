...
public abstract class DeviceModule {
    ...
    @Provides
    static BluetoothDevice provideBluetoothDevice(@Named(MAC_ADDRESS) String str, RxBleAdapterWrapper rxBleAdapterWrapper) {
        return rxBleAdapterWrapper.getRemoteDevice(str);
    }

    @Named(CONNECT_TIMEOUT)
    @Provides
    static TimeoutConfiguration providesConnectTimeoutConf(@Named(ClientComponent.NamedSchedulers.TIMEOUT) Scheduler scheduler) {
        return new TimeoutConfiguration(35L, TimeUnit.SECONDS, scheduler);
    }

    @Named(DISCONNECT_TIMEOUT)
    @Provides
    static TimeoutConfiguration providesDisconnectTimeoutConf(@Named(ClientComponent.NamedSchedulers.TIMEOUT) Scheduler scheduler) {
