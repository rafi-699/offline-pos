...
public abstract class ConnectionModule {
    ...
    @Provides
    static TimeoutConfiguration providesOperationTimeoutConf(@Named(ClientComponent.NamedSchedulers.TIMEOUT) Scheduler scheduler, Timeout timeout) {
        return new TimeoutConfiguration(timeout.timeout, timeout.timeUnit, scheduler);
    }

    @Provides
    static IllegalOperationHandler provideIllegalOperationHandler(@Named(ConnectionComponent.NamedBooleans.SUPPRESS_OPERATION_CHECKS) boolean z, Provider<LoggingIllegalOperationHandler> provider, Provider<ThrowingIllegalOperationHandler> provider2) {
        if (z) {
            return provider.get();
        }
