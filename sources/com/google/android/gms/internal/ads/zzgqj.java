...
public final class zzgqj {
    ...
    private static Intent zzc(Intent intent, int i, int i2) {
        ...
        if (zzd(0, 1)) {
            zzgtj.zzb(!zzd(i, AccessibilityEventCompat.TYPE_VIEW_TARGETED_BY_SCROLL), "Cannot set mutability flags if PendingIntent.FLAG_IMMUTABLE is set.");
        } else {
            zzgtj.zzb(zzd(i, AccessibilityEventCompat.TYPE_VIEW_TARGETED_BY_SCROLL), "Must set PendingIntent.FLAG_IMMUTABLE for SDK >= 23 if no parts of intent are mutable.");
        }
