...
public final class Contrast {
    ...
    public static double lighter(double d, double d2) {
        if (d >= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE && d <= 100.0d) {
            double dYFromLstar = ColorUtils.yFromLstar(d);
            double d3 = ((dYFromLstar + 5.0d) * d2) - 5.0d;
            if (d3 >= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE && d3 <= 100.0d) {
                ...
                if (dLstarFromY >= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE && dLstarFromY <= 100.0d) {
                    return dLstarFromY;
                }
            }
        }
    ...
    public static double lighterUnsafe(double d, double d2) {
        ...
        if (dLighter < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            return 100.0d;
        }
        return dLighter;
    }
    ...
    public static double darkerUnsafe(double d, double d2) {
        return Math.max(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, darker(d, d2));
    }
}
