...
public class HctSolver {
    ...
    static boolean isBounded(double d) {
        return ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE <= d && d <= 100.0d;
    }
    ...
    static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (dArrNthVertex[0] < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
                d3 = dHueOf;
            } else {
    ...
    static double inverseChromaticAdaptation(double d) {
        ...
        return ((double) MathUtils.signum(d)) * Math.pow(Math.max(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }
    ...
    static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            double d8 = d5;
            double d9 = dSqrt;
            double dPow2 = Math.pow(((d2 == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE || dSqrt == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) ? 0.0d : d2 / Math.sqrt(d7)) * dPow, 1.1111111111111112d);
            ...
            if (d14 < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
                break;
            }
            double d15 = dArrMatrixMultiply[1];
            if (d15 < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
                break;
            }
            double d16 = dArrMatrixMultiply[2];
            if (d16 >= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
                double[] dArr = Y_FROM_LINRGB;
                double d17 = (dArr[0] * d14) + (dArr[1] * d15) + (dArr[2] * d16);
                if (d17 > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
                    if (i != 4) {
                        double d18 = d17 - d3;
