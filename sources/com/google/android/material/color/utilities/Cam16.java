...
public final class Cam16 {
    ...
    private final double[] tempArray = {ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE};
    ...
    static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        if (degrees < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            degrees += 360.0d;
    ...
    public static Cam16 fromUcsInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        if (dAtan2 < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            dAtan2 += 360.0d;
        }
    ...
    double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] dArr) {
        double dPow = Math.pow(((getChroma() == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE || getJ() == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) ? 0.0d : getChroma() / Math.sqrt(getJ() / 100.0d)) / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d), 1.1111111111111112d);
        ...
        double dSignum = Math.signum(d5) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, (Math.abs(d5) * 27.13d) / (400.0d - Math.abs(d5))), 2.380952380952381d);
        double dSignum2 = Math.signum(d6) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, (Math.abs(d6) * 27.13d) / (400.0d - Math.abs(d6))), 2.380952380952381d);
        double dSignum3 = Math.signum(d7) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, (Math.abs(d7) * 27.13d) / (400.0d - Math.abs(d7))), 2.380952380952381d);
