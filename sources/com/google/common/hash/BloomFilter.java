...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    static <T> BloomFilter<T> create(Funnel<? super T> funnel, long expectedInsertions, double fpp, Strategy strategy) {
        ...
        Preconditions.checkArgument(fpp > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, "False positive probability (%s) must be > 0.0", Double.valueOf(fpp));
    ...
    static long optimalNumOfBits(long n, double p) {
        if (p == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            p = Double.MIN_VALUE;
        }
