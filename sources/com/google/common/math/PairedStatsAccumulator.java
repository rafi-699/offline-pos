...
public final class PairedStatsAccumulator {
    ...
    private double sumOfProductsOfDeltas = ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE;

    private double ensurePositive(double value) {
        if (value > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            return value;
        }
    ...
    public final double pearsonsCorrelationCoefficient() {
        ...
        Preconditions.checkState(dSumOfSquaresOfDeltas > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE);
        Preconditions.checkState(dSumOfSquaresOfDeltas2 > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE);
    ...
    public final LinearTransformation leastSquaresFit() {
        ...
        double dSumOfSquaresOfDeltas = this.xStats.sumOfSquaresOfDeltas();
        if (dSumOfSquaresOfDeltas <= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            Preconditions.checkState(this.yStats.sumOfSquaresOfDeltas() > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE);
            return LinearTransformation.vertical(this.xStats.mean());
        }
        if (this.yStats.sumOfSquaresOfDeltas() > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
