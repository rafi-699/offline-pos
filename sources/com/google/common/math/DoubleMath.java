...
public final class DoubleMath {
    ...
    static double roundIntermediate(double x, RoundingMode mode) {
        ...
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[mode.ordinal()]) {
            case 1:
                ...
                return x;
            case 2:
                return (x >= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE || isMathematicalInteger(x)) ? x : ((long) x) - 1;
            case 3:
                return (x <= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE || isMathematicalInteger(x)) ? x : ((long) x) + 1;
            case 4:
                return x;
            case 5:
                ...
                return ((long) x) + ((long) (x > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE ? 1 : -1));
            case 6:
                return Math.rint(x);
            case 7:
    ...
    public static BigInteger roundToBigInteger(double x, RoundingMode mode) {
        ...
        return dRoundIntermediate < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE ? bigIntegerShiftLeft.negate() : bigIntegerShiftLeft;
    }

    public static boolean isPowerOfTwo(double x) {
        if (x > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE && DoubleUtils.isFinite(x)) {
    ...
    public static int log2(double x, RoundingMode mode) {
        ...
        boolean z = false;
        Preconditions.checkArgument(x > ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE && DoubleUtils.isFinite(x), "x must be positive and finite");
    ...
    public static boolean isMathematicalInteger(double x) {
        if (DoubleUtils.isFinite(x)) {
            return x == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE || 52 - Long.numberOfTrailingZeros(DoubleUtils.getSignificand(x)) <= Math.getExponent(x);
        }
        return false;
    }
