...
class Brush {
    ...
    private double getVal(SVGLength sVGLength, double d, float f, float f2) {
        return PropHelper.fromRelative(sVGLength, d, ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, (this.mUseObjectBoundingBox && sVGLength.unit == SVGLength.UnitType.NUMBER) ? d : f, f2);
    }
    ...
    void setupPaint(Paint paint, RectF rectF, float f, float f2) {
        ...
        if (this.mType == BrushType.RADIAL_GRADIENT) {
            ...
            if (val6 <= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE || val7 <= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            ...
            double d10 = d / val6;
            Shader radialGradient = new RadialGradient((float) (getVal(this.mPoints[4], d8, f, textSize) + ((double) f3)), (float) (getVal(this.mPoints[5], d9 / d10, f, textSize) + (((double) f4) / d10)), (float) val6, iArr2, fArr2, Shader.TileMode.CLAMP);
