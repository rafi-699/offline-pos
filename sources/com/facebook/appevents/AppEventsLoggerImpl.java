            ...
            if (str != null && str.length() != 0) {
                if (!isImplicitlyLogged && AutomaticAnalyticsLogger.isImplicitPurchaseLoggingEnabled() && (Intrinsics.areEqual(eventName, AppEventsConstants.EVENT_NAME_PURCHASED) || Intrinsics.areEqual(eventName, AppEventsConstants.EVENT_NAME_SUBSCRIBE) || Intrinsics.areEqual(eventName, AppEventsConstants.EVENT_NAME_START_TRIAL))) {
                    ...
                    if ((FeatureManager.isEnabled(FeatureManager.Feature.AndroidManualImplicitPurchaseDedupe) && Intrinsics.areEqual(eventName, AppEventsConstants.EVENT_NAME_PURCHASED)) || (FeatureManager.isEnabled(FeatureManager.Feature.AndroidManualImplicitSubsDedupe) && (Intrinsics.areEqual(eventName, AppEventsConstants.EVENT_NAME_SUBSCRIBE) || Intrinsics.areEqual(eventName, AppEventsConstants.EVENT_NAME_START_TRIAL)))) {
                        ...
                        if (valueOfManualEvent == null || currencyOfManualEvent == null) {
                            ...
                            Pair<Bundle, OperationalData> pairAddDedupeParameters = InAppPurchaseDedupeConfig.INSTANCE.addDedupeParameters(InAppPurchaseManager.performDedupe(CollectionsKt.listOf(new InAppPurchase(eventName, valueOfManualEvent.doubleValue(), currencyOfManualEvent)), System.currentTimeMillis(), false, CollectionsKt.listOf(new Pair(parameters, operationalDataComponent2))), parameters, operationalDataComponent2);
                ...
                try {
                    if (!ProtectedModeManager.INSTANCE.protectedModeIsApplied(bundleComponent2)) {
                        SensitiveParamsManager.processFilterSensitiveParams(bundleComponent2, eventName);
                    }
                    BannedParamManager.processFilterBannedParams(bundleComponent2);
                    MACARuleMatchingManager.processParameters(bundleComponent2, eventName);
                    ...
                    VVPManager.processParametersForVVP(eventName, bundleComponent2);
                    companion.logEvent(new AppEvent(this.contextName, eventName, valueToSum, bundleComponent2, isImplicitlyLogged, ActivityLifecycleTracker.isInBackground(), currentSessionId, operationalDataComponent3), this.accessTokenAppId);
