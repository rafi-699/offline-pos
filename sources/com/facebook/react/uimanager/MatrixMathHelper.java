...
public final class MatrixMathHelper {
    ...
    public static final double degreesToRadians(double degrees) {
        return (degrees * 3.141592653589793d) / ((double) RotationOptions.ROTATE_180);
    }
    ...
    /* JADX WARN: Code duplicated, block: B:37:0x00d1 A[LOOP:3: B:36:0x00cf->B:37:0x00d1, LOOP_END] */
    /* JADX WARN: Code duplicated, block: B:40:0x00e0 A[LOOP:4: B:39:0x00de->B:40:0x00e0, LOOP_END] */
    /* JADX WARN: Code duplicated, block: B:43:0x00eb A[LOOP:5: B:42:0x00e9->B:43:0x00eb, LOOP_END] */
    /* JADX WARN: Code duplicated, block: B:46:0x0190  */
    /* JADX WARN: Code duplicated, block: B:48:0x0194 A[LOOP:6: B:47:0x0192->B:48:0x0194, LOOP_END] */
    @JvmStatic
    public static final void decomposeMatrix(double[] transformMatrix, MatrixDecompositionContext ctx) {
        ...
        for (int i5 = 0; i5 < 4; i5++) {
            dArr7[i5] = new double[4];
        }
        double[] dArr8 = new double[16];
        for (int i6 = 0; i6 < 4; i6++) {
            for (int i7 = 0; i7 < 4; i7++) {
                int i8 = (i6 * 4) + i7;
                double d = transformMatrix[i8] / transformMatrix[15];
                dArr7[i6][i7] = d;
                if (i7 == 3) {
                    d = 0.0d;
                }
        ...
        if (matrixMathHelper.isZero(dArr7[0][3])) {
            ...
            double dV3Dot2 = v3Dot(dArr[0], dArr[c]);
            dArr4[1] = dV3Dot2;
            double[] dArrV3Combine2 = v3Combine(dArr[c], dArr[0], 1.0d, -dV3Dot2);
            dArr[c] = dArrV3Combine2;
            double dV3Dot3 = v3Dot(dArr[1], dArrV3Combine2);
            dArr4[c] = dV3Dot3;
            double[] dArrV3Combine3 = v3Combine(dArr[c], dArr[1], 1.0d, -dV3Dot3);
            dArr[c] = dArrV3Combine3;
            double dV3Length3 = v3Length(dArrV3Combine3);
            dArr3[c] = dV3Length3;
            dArrV3Normalize = v3Normalize(dArr[c], dV3Length3);
            dArr[c] = dArrV3Normalize;
            double d2 = dArr4[1];
            double d3 = dArr3[c];
            dArr4[1] = d2 / d3;
            dArr4[c] = dArr4[c] / d3;
            if (v3Dot(dArr[0], v3Cross(dArr[1], dArrV3Normalize)) < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
                for (i4 = 0; i4 < 3; i4++) {
                    dArr3[i4] = dArr3[i4] * (-1.0d);
                    double[] dArr11 = dArr[i4];
        ...
        while (i3 < 3) {
            double[] dArr15 = dArr[i3];
            double[] dArr16 = dArr7[i3];
            dArr15[0] = dArr16[0];
            dArr15[1] = dArr16[1];
            dArr15[c] = dArr16[c];
        }
        ...
        dArr4[0] = dArr4[0] / dArr3[1];
        double dV3Dot5 = v3Dot(dArr[0], dArr[c]);
        dArr4[1] = dV3Dot5;
        double[] dArrV3Combine5 = v3Combine(dArr[c], dArr[0], 1.0d, -dV3Dot5);
        dArr[c] = dArrV3Combine5;
        double dV3Dot6 = v3Dot(dArr[1], dArrV3Combine5);
        dArr4[c] = dV3Dot6;
        double[] dArrV3Combine6 = v3Combine(dArr[c], dArr[1], 1.0d, -dV3Dot6);
        dArr[c] = dArrV3Combine6;
        double dV3Length6 = v3Length(dArrV3Combine6);
        dArr3[c] = dV3Length6;
        dArrV3Normalize = v3Normalize(dArr[c], dV3Length6);
        dArr[c] = dArrV3Normalize;
        double d11 = dArr4[1];
        double d12 = dArr3[c];
        dArr4[1] = d11 / d12;
        ...
        if (v3Dot(dArr[0], v3Cross(dArr[1], dArrV3Normalize)) < ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            while (i4 < 3) {
                ...
                dArr17[c] = dArr17[c] * (-1.0d);
            }
        }
        double[] dArr18 = dArr[c];
        dArr6[0] = roundTo3Places((-Math.atan2(dArr18[1], dArr18[c])) * 57.29577951308232d);
        double[] dArr19 = dArr[c];
        double d13 = -dArr19[0];
        double d14 = dArr19[1];
        double d15 = dArr19[c];
