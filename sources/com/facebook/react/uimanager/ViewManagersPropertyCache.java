    ...
    private static void createPropSetters(ReactPropGroup reactPropGroup, Method method, Class<?> cls, Map<String, PropSetter> map) {
        ...
        if (cls == Dynamic.class) {
            while (i < strArrNames.length) {
                map.put(strArrNames[i], new DynamicPropSetter(reactPropGroup, method, i));
                i++;
            }
            return;
        }
        if (cls == Integer.TYPE) {
            while (i < strArrNames.length) {
                if ("Color".equals(reactPropGroup.customType())) {
                    map.put(strArrNames[i], new ColorPropSetter(reactPropGroup, method, i, reactPropGroup.defaultInt()));
                } else {
                    map.put(strArrNames[i], new IntPropSetter(reactPropGroup, method, i, reactPropGroup.defaultInt()));
                }
                i++;
            }
            return;
        ...
        if (cls == Float.TYPE) {
            while (i < strArrNames.length) {
                map.put(strArrNames[i], new FloatPropSetter(reactPropGroup, method, i, reactPropGroup.defaultFloat()));
                i++;
            }
        } else {
            if (cls == Double.TYPE) {
            ...
            if (cls == Integer.class) {
                while (i < strArrNames.length) {
                    if ("Color".equals(reactPropGroup.customType())) {
                        map.put(strArrNames[i], new BoxedColorPropSetter(reactPropGroup, method, i));
                    } else {
                        map.put(strArrNames[i], new BoxedIntPropSetter(reactPropGroup, method, i));
                    }
                    i++;
                }
                return;
