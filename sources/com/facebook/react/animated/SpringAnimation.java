...
public final class SpringAnimation extends AnimationDriver {
    ...
    public SpringAnimation(ReadableMap config) {
        ...
        PhysicsState physicsState = new PhysicsState(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, 3, null);
    ...
    private static final /* data */ class PhysicsState {
        ...
        public PhysicsState() {
            this(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE, 3, null);
        }
    ...
    private final boolean isAtRest() {
        if (Math.abs(this.currentState.getVelocity()) <= this.restSpeedThreshold) {
            return getDisplacementDistanceForState(this.currentState) <= this.displacementFromRestThreshold || this.springStiffness == ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE;
        }
        return false;
    }

    private final boolean isOvershooting() {
        if (this.springStiffness <= ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE) {
            return false;
        }
    ...
    public void resetConfig(ReadableMap config) {
        ...
        this.currentLoop = 0;
        this.timeAccumulator = ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE;
    ...
    private final void advance(double realDeltaTime) {
        ...
        double dSqrt = d2 / (((double) 2) * Math.sqrt(d4 * d3));
        ...
        if (dSqrt < 1.0d) {
            ...
            double d8 = dSqrt2 * dSqrt;
            double d9 = d5 + (d8 * d6);
            double d10 = d7 * dSqrt3;
            dSin2 = this.endValue - ((((d9 / dSqrt3) * Math.sin(d10)) + (Math.cos(d10) * d6)) * dExp);
            dSin = ((d8 * dExp) * (((Math.sin(d10) * d9) / dSqrt3) + (Math.cos(d10) * d6))) - (dExp * ((Math.cos(d10) * d9) - ((dSqrt3 * d6) * Math.sin(d10))));
        } else {
            double dExp2 = Math.exp((-dSqrt2) * d7);
            double d11 = this.endValue - (((((dSqrt2 * d6) + d5) * d7) + d6) * dExp2);
            dSin = dExp2 * ((d5 * ((d7 * dSqrt2) - ((double) 1))) + (d7 * d6 * dSqrt2 * dSqrt2));
            dSin2 = d11;
        }
        ...
        if (isAtRest() || (this.overshootClampingEnabled && isOvershooting())) {
            ...
            this.currentState.setVelocity(ReactExoplayerView.DEFAULT_MIN_BUFFER_MEMORY_RESERVE);
        }
    }
}
