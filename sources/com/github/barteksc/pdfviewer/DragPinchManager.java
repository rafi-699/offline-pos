...
class DragPinchManager implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener, ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    private boolean checkLinkTapped(float f, float f2) {
        ...
        for (PdfDocument.Link link : pdfFile.getPageLinks(pageAtOffset)) {
            ...
            if (rectFMapRectToDevice.contains(f3, f4)) {
                this.pdfView.callbacks.callLinkHandler(new LinkTapEvent(f, f2, f3, f4, rectFMapRectToDevice, link));
                return true;
            }
        }
        return false;
    ...
    private void startPageFling(MotionEvent motionEvent, MotionEvent motionEvent2, float f, float f2) {
        ...
        if (checkDoPageFling(f, f2)) {
            ...
            float f3 = x - x2;
            int iMax = Math.max(0, Math.min(this.pdfView.getPageCount() - 1, this.pdfView.findFocusPage(this.pdfView.getCurrentXOffset() - (this.pdfView.getZoom() * f3), this.pdfView.getCurrentYOffset() - (f3 * this.pdfView.getZoom())) + i));
    ...
    private void onBoundedFling(float f, float f2) {
        ...
        int currentYOffset = (int) this.pdfView.getCurrentYOffset();
        PdfFile pdfFile = this.pdfView.pdfFile;
        float f4 = -pdfFile.getPageOffset(this.pdfView.getCurrentPage(), this.pdfView.getZoom());
        float pageLength = f4 - pdfFile.getPageLength(this.pdfView.getCurrentPage(), this.pdfView.getZoom());
        float f5 = 0.0f;
        if (this.pdfView.isSwipeVertical()) {
            f3 = -(this.pdfView.toCurrentScale(pdfFile.getMaxPageWidth()) - this.pdfView.getWidth());
        ...
        this.animationManager.startFlingAnimation(currentXOffset, currentYOffset, (int) f, (int) f2, (int) f3, (int) f4, (int) height, (int) f5);
    }
