    ...
    public static final class Factory implements ChunkExtractor.Factory {
        ...
        public Format getOutputTextFormat(Format format) {
            if (this.parseSubtitlesDuringExtraction && this.subtitleParserFactory.supportsFormat(format)) {
                return format.buildUpon().setSampleMimeType(MimeTypes.APPLICATION_MEDIA3_CUES).setCueReplacementBehavior(this.subtitleParserFactory.getCueReplacementBehavior(format)).setCodecs(format.sampleMimeType + (format.codecs != null ? StringUtils.SPACE + format.codecs : "")).setSubsampleOffsetUs(Long.MAX_VALUE).build();
            }
            return format;
        }
