...
class FragmentStateManager {
    ...
    void resume() {
        ...
        if (focusedView != null && isFragmentViewChild(focusedView)) {
            ...
            if (FragmentManager.isLoggingEnabled(2)) {
                Log.v("FragmentManager", "requestFocus: Restoring focused view " + focusedView + StringUtils.SPACE + (zRequestFocus ? AnalyticsEvents.PARAMETER_SHARE_OUTCOME_SUCCEEDED : "failed") + " on Fragment " + this.mFragment + " resulting in focused view " + this.mFragment.mView.findFocus());
            }
        }
    ...
    Bundle saveState() {
        ...
        if (this.mFragment.mState > -1) {
            ...
            Bundle bundleM161lambda$attachController$4$androidxfragmentappFragmentManager = this.mFragment.mChildFragmentManager.m161lambda$attachController$4$androidxfragmentappFragmentManager();
            if (!bundleM161lambda$attachController$4$androidxfragmentappFragmentManager.isEmpty()) {
                bundle.putBundle(CHILD_FRAGMENT_MANAGER_KEY, bundleM161lambda$attachController$4$androidxfragmentappFragmentManager);
            }
            if (this.mFragment.mView != null) {
                saveViewState();
            }
